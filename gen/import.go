...
func (m *pulumiModule) importResourceSDK() (*resourceSDK, hcl.Diagnostics) {
	if m.resourceSDK != nil {
		if m.resourceSDK.types == nil {
			return nil, nil
		}
		return m.resourceSDK, nil
	}

	packageName := m.goPackage.Syntax[0].Name

	sdk, ok := findPackage("github.com/pulumi/pulumi/sdk/v2/go/common/resource", m.goPackage.Types.Imports())
	if !ok {
		m.resourceSDK = &resourceSDK{}
	...
	if diags.HasErrors() {
		m.resourceSDK.types = nil
	}
	return m.resourceSDK, diags
}
...
//     interface {
//         Args() *ArgsType
//         Create(provider *ProviderType, args *ArgsType, options OptionsType) (id IdType, err error)
//         Read(provider *ProviderType, id IdType, options OptionsType) error
//         Update(provider *ProviderType, id IdType, args *ArgsType, options OptionsType) error
//         Delete(provider *ProviderType, id IdType, options OptionsType) error
//     }
//
// ProviderType is the provider type for the package. ArgsType is determined by the type of the second parameters
// to the Create method. It is an error if no Create method is present. OptionsType and IdType are types from the
// Pulumi provider SDK (NYI).
func (m *pulumiModule) importResource(resource *pulumiResource) hcl.Diagnostics {
	...
	sdk, sdkDiags := m.importResourceSDK()
