...
import (
	...
	_ "github.com/pgavlin/go-provider/gen/statik"
)
...
func loadTemplates() (*template.Template, error) {
	...
	loadTemplate := func(name string) error {
		f, err := fs.Open("/" + name + ".tmpl")
		if err != nil {
			return err
		}
		defer f.Close()
		...
		_, err = templates.New(name).Parse(string(contents))
		return err
	}

	for _, name := range templateNames {
		if err = loadTemplate(name); err != nil {
			return nil, fmt.Errorf("failed to load template %v: %v", name, err)
		}
	}
...
func (p *pulumiPackage) genProvider(files map[string][]byte, schema schema.PackageSpec) error {
	...
	for _, templateName := range templateNames {
		...
		if err = templates.ExecuteTemplate(&contents, templateName, context); err != nil {
			return err
		}
...
func GenerateProvider(name, rootImport string, packages ...*packages.Package) (map[string][]byte, hcl.Diagnostics) {
	pp := &pulumiPackage{
		name:        name,
		typeClosure: typeSet{},
		outputTypes: typeSet{},
